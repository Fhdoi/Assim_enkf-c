...
class CiceBry(object):
    ...
    def verify_aice_below_one(self):
        """Method that checks if bry file contains any point in space or time where sum(aicen)>1.
        If that is the case, sum(aicen) for that point is hacked down to below or equal to 1.
        should be calles after self.write_bry_data() and before self.expand_to_yearly_files()."""
        print("Checking for sum(aicen) > 1 and fixing it...")
        if not self.data_written_to_bry:
            raise RuntimeError("Must have called self.write_bry_data() before calling verify_aice_below_one()!")
        
        fixed_anything = False

        # loop explicitly over time dimension to support large arrays (memory)
        for i in range(self.bry.dimensions["days"].size):
            for bry in self.bry_order:
                var_name = "aicen_{}_bry".format(bry)
                arr = self.bry.variables[var_name][i,:,:].copy()
                s = np.sum(arr[:,:], axis=0)
                s_above_one = np.where(s>1)

                # if any points along the boundary has sum(aicen)>1, then remove aice from cat 0 to make aggregate <= 1
                if s_above_one[0].size > 0:
                    print("\tsum({}) > 1 detected at {}! Fixing...".format(var_name, netCDF4.num2date(self.bry.variables["Time"][i], self.bry.variables["Time"].units)))
                    arr[0,s_above_one] -= (s[s_above_one] - 1)  # could be more fancy than this

                    if np.any(arr < 0):
                        raise ValueError("{} values below zero detected, method failed!".format(var_name))
                    elif np.any(np.sum(arr, axis=0) > 1):
                        raise ValueError("{} still above one after fix, method failed!".format(var_name))

                    self.bry.variables[var_name][i,:,:] = arr
                    fixed_anything = True
        
        if fixed_anything:
            print("All sum(aicen)>1 fixed successfully!")
        else:
            print("Did not need to fix anything!")
    ...
    def expand_to_yearly_files(self):
        ...
        """
        print("Expanding {} into a yearly boundary files...".format(self.bry_file))

        if not self.data_written_to_bry:
            raise RuntimeError("Must have called self.write_bry_data() before epxanding to yearly files!")

        # split date range on bry file in to list of list, one list for each year
        time_on_file = self.bry.variables["Time"]
        dates_on_file = netCDF4.num2date(time_on_file[:], time_on_file.units)
        year_lists = list()  # to contain present dates for each year in self.bry_file
        year_dates = list()  # refilled below for each year in dates_on_file

        for d in dates_on_file:
            if d.month == 1 and d.day == 1 and year_dates:
                year_lists.append(year_dates)
                year_dates = [d]  # reset current year
            else:
                year_dates.append(d)

        print(year_dates)
        year_lists.append(year_dates)  # add final year_dates

        # get fill data for yearly files
        fill_pre_range = dict()
        fill_in_range = dict()
        fill_post_range = dict()

        for var_name, var in self.bry.variables.items():
        ...
        output_template = "BRY_{}.nc"
        ref_date = dt.datetime.strptime(time_on_file.units, "days since %Y-%m-%d %H:%M:%S")
        
        print(year_lists)
        print(year_lists[0][0])
        total_days = (dt.datetime(year_lists[0][0].year, 1, 1) - ref_date).days

        # loop over year-by-year lists and generate a boundary file for each year
        for ydates in year_lists:
            work_dir = "/".join(self.bry_file.split("/")[:-1])  # extract path excepth actual filename
            filename = os.path.join(work_dir, output_template.format(ydates[0].year))  # e.g. BRY_2019.nc
            shutil.copyfile(self.bry_file, filename)
            ds = netCDF4.Dataset(filename, mode="r+")
            print("\tWriting {}...".format(filename))

            days_in_year = 366 if calendar.isleap(ydates[0].year) else 365
            start_date_idx = (ydates[0] - dt.datetime(ydates[0].year, 1, 1)).days
            end_date_idx = (ydates[-1] - dt.datetime(ydates[0].year, 1, 1)).days
            ds.variables["Time"].units = dt.datetime(ydates[0].year, 1, 1).strftime("days since %Y-%m-%d %H:%M:%S")  # set units relative to Jan 1st of current year

            # loop over 365/366 days in current year and fill with data from original self.bry_file
            for n in range(days_in_year):
                current_date = ref_date + dt.timedelta(days=total_days)
                ...
                for var_name in fill_pre_range.keys():
                    if n < start_date_idx:
                        ...
                        n_offset = (current_date - year_lists[0][0]).days  # index in original bry file to fetch data
...
class CiceBryCiceHax(CiceBry):
    ...
    Example usage:
        > ...
    """
    def __init__(self, bry_file, grid_file, cice_file, mode="w", fmt="NETCDF3_CLASSIC", overwrite=True):
        """Constructor setting attributes and opening all input files for reading.
    ...
    def get_cice_times(self):
        ...
        """
        cice_times = list()
        patterns = {"%Y%m%d": "\d{4}\d{2}\d{2}", "%Y-%m-%d": "\d{4}-\d{2}-\d{2}"}

        for fn in self.cice_files:
            for date_fmt, p in patterns.items():
                m = re.search(p, fn)

                if m is not None:
                    cice_times.append(dt.datetime.strptime(m.group(), date_fmt))
                    break
            else:
                raise ValueError("Filename must contain date string on one of the formats: {}".format(patterns))

        return np.array(cice_times)

    def get_slice(self, slice_type, bry):
        """
        Method that gives a tuple of indexes/slices to be used for indexing the
        cice restart files when extracting data.
    ...
    def write_bry_data(self):
        """
        Method that uses a CICE restart file to write a cice boundary file by copying all data
        along the boundaries of the restart file.
        """
        if self.grid is None:
            raise AttributeError("No grid file has been set! Use self.set_grid().")

        if None in (self.start_date, self.end_date):
            raise ValueError("Need to set start_date and end_date (set_dates())")

        # get time arrays needed for loop below
        super(CiceBryCiceHax, self).set_var_attr("Time", "units", self.time_units, update_file=True)  # set new time units for Time var
        bry_dates = [self.start_date + dt.timedelta(days=i) for i in range((self.end_date-self.start_date).days+1)]  # date range between start and end
        bry_time = netCDF4.date2num(bry_dates, self.time_units)  # float array to be written to bry_file
        cice_dates = self.get_cice_times()  # date array for cice files
        print(cice_dates)

        if not all(d in cice_dates for d in bry_dates):
            raise ValueError("CICE files does not cover all dates {}!".format(date_range))

        land_mask = self.grid.variables["mask_rho"]  # grid file land mask

        zero_vars_2d = ["iage_BRY_bry"]  # variables that, due to lack of better data, is set to zero everywhere
        zero_vars_3d = ["fbrine_BRY_bry", "hbrine_BRY_bry"]  # variables that, due to lack of better data, is set to zero everywhere
        cice_vars_3d = ["aicen_BRY_bry", "vicen_BRY_bry", "vsnon_BRY_bry", "Tsfc_BRY_bry", "apondn_BRY_bry",
                        "hpondn_BRY_bry", "ipondn_BRY_bry", "alvln_BRY_bry", "vlvln_BRY_bry"]  # to be filled by CICE restart files

        # write time-independent data to boundary file
        self.bry.variables["Ice_layers"][:] = list(range(self.dims["nkice"]))  # vertical dim variable
        for bry in self.bry_order:
            grid_slice = self.get_slice("cice_2d", bry=bry)
            self.bry.variables["TLON_{}".format(bry)][:] = self.grid.variables["lon_rho"][grid_slice]  # longitude
            self.bry.variables["TLAT_{}".format(bry)][:] = self.grid.variables["lat_rho"][grid_slice]  # latitude
        ...
        # handle one time index at a time for memory efficiency and write boundary data for all times,
        # all ice categories and all four boundaries
        for bry_tidx in range(len(bry_dates)):
            self.bry.variables["Time"][bry_tidx] = bry_time[bry_tidx]  # fill time array with current time
            cice_tidx = (np.abs(cice_dates - bry_dates[bry_tidx])).argmin()  # closest CICE rst file to bry_dates[bry_tidx]
            cice = netCDF4.Dataset(self.cice_files[cice_tidx], mode="r")  # cice rst file to use for current time
            ice_mask = np.where(cice.variables["aicen"][:]*land_mask[:,:] > 0.0, 1, 0)  # to filter out areas with no ice below
            print("\tFile: {}\n\tbry time: {},\n\tcice time: {}".format(self.cice_files[cice_tidx], bry_dates[bry_tidx], cice_dates[cice_tidx]))

            # loop over all 4 boundaries (W, S, E, N)
            for bry in self.bry_order:
                print("\t\tBoundary {}".format(bry))
                bslice_2d = (bry_tidx, slice(None))         # slices for 2D bry variables (time, xi/eta)
                cice_slice_2d = self.get_slice("cice_2d", bry=bry)  # slice (x,y) for cice forecast data

                # loop over all ice categories and fill from clm and atm data
                for n in range(len(self.cat_lims[1:])):
                    print("\t\t\tCategory {}".format(n))
                    bslice_3d = (bry_tidx, n, slice(None))      # slices for 3D bry variables (time, cat, xi/eta)
                    cice_slice_3d = tuple([n] + list(self.get_slice("cice_2d", bry=bry)))  # slice (x,y) for cice forecast data

                    # remove areas with very little or thin ice as well as make all cie have at least some snow to improve model stability
                    aice_mask = cice.variables["aicen"][cice_slice_3d] > 0.01
                    hice_mask = cice.variables["vicen"][cice_slice_3d] > 0.01
                    valid_mask = np.logical_and(aice_mask, hice_mask)
                    snow_limit = 0.01
                    snow_hacked = np.where(cice.variables["vsnon"][cice_slice_3d] > snow_limit, cice.variables["vsnon"][cice_slice_3d], snow_limit)

                    # copy most variables from CICE rst file to BRY file
                    for var in cice_vars_3d:
                        bry_name = var.replace("BRY", bry)
                        cice_name = self.cice_names[var.split("_")[0]]

                        if cice_name == "vsnon":
                            self.bry.variables[bry_name][bslice_3d] = np.where(valid_mask, snow_hacked, 0.0)
                        else:
                            self.bry.variables[bry_name][bslice_3d] = np.where(valid_mask, cice.variables[cice_name][cice_slice_3d], 0.0)

                    # copy vertical salinity profile (4d var) from CICE rst file
                    for zlvl in range(self.dims["nkice"]):
                        bslice_z = (bry_tidx, n, zlvl, slice(None))
                        sinz_cice_name = "sice00{}".format(zlvl+1)        # one sice variable per zlvl on input file
                        sinz_bry_name = "Sinz_{}_bry".format(bry)
                        qice_cice_name = "qice00{}".format(zlvl+1)        # one qice variable per zlvl on input file
                        tinz_bry_name = "Tinz_{}_bry".format(bry)
                        self.bry.variables[sinz_bry_name][bslice_z] = np.where(valid_mask, cice.variables[sinz_cice_name][cice_slice_3d], 0.0)*land_mask[cice_slice_2d]
                        self.bry.variables[tinz_bry_name][bslice_z] = temperature_mush(np.where(valid_mask, cice.variables[qice_cice_name][cice_slice_3d], 0.0),
                                                                                       np.where(valid_mask, cice.variables[sinz_cice_name][cice_slice_3d], 0.0))*land_mask[cice_slice_2d]
