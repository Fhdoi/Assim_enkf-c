...
def count_ens(enkf_c_dir, res_dir):
    file_ens = open(enkf_c_dir+'files_in_ensemble', "w")
    files = glob.glob(res_dir+'iced.*')
    for file in files:
        sens = file[-6:-3]
...
def Prep_ensemble(ens_date, grid_dir, ens_inn_dir, enkf_c_dir, res_type, EnKF_var,Nens):
    ...
    file_cord_handle = xr.open_dataset(grid_dir)
        ...
        if os.path.exists(file):
            ...
            for var in file_handle.variables.keys():
                ...
                if var in EnKF_var:
                    ...
                    fn = enkf_c_dir+'ensemble_6565/mem'+s1ens+'_'+var+'.nc'
                    ...
                    # the surface if possible, Johannes should know more about this
                        ...
                        if var == 'temp':
                            var2 = 'sst'
                            fn = enkf_c_dir+'ensemble_6565/mem'+s1ens+'_sst.nc'
                        if var == 'salt':
                            var2 = 'sss'
                            fn = enkf_c_dir+'ensemble_6565/mem'+s1ens+'_sss.nc'
                        ds = nc.Dataset(fn, 'w', format='NETCDF4')
                    ...
                    # Write also integrated variables for assimilation
                        ...
                        if var == 'aicen':
                            var2 = 'aice'
                            fn = enkf_c_dir+'ensemble_6565/mem'+s1ens+'_aice.nc'
                        elif var == 'vicen':
                            var2 = 'vice'
                            fn = enkf_c_dir+'ensemble_6565/mem'+s1ens+'_vice.nc'
                        ds = nc.Dataset(fn, 'w', format='NETCDF4')
                    ...
                    # Write smoothed variable for low resolution assimilation
                        ...
                        fn = enkf_c_dir+'ensemble_6565/mem'+s1ens+'_aiceosi.nc'
                        ds = nc.Dataset(fn, 'w', format='NETCDF4')
...
def write_res(ll,enkf_c_dir, EnKF_var,ens_out_dir,ens_date):
    prescripts = ['iced.','ocean.']
    syear = str(ens_date.year)
    smnd = str(ens_date.month)
    if ens_date.month < 10:
