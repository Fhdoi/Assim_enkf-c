...
noup: clean enkf-prep enkf-calc
...
enkf-calc: enkf_calc observations.nc conf ensemble_6565
	nice -n +10 mpirun -np $(NP_CALC) ./enkf_calc enkf.prm 2>&1 | tee calc.out

enkf-update: enkf_update X5-0.nc X5-1.nc conf ensemble_6565
	nice -n +10 mpirun -np $(NP_UPDATE) ./enkf_update --calculate-spread --write-inflation enkf.prm 2>&1 | tee update.out
...
enoi-calc: enkf_calc observations.nc conf background_6565 ensemble_static
	nice -n +10 mpirun -np $(NP_CALC) ./enkf_calc enoi.prm 2>&1 | tee calc.out

enoi-update: enkf_update w.nc conf background_6565 ensemble_static
	nice -n +10 mpirun -np $(NP_UPDATE) ./enkf_update --calculate-spread --output-increment enoi.prm 2>&1 | tee update.out
...
stats-calc: enkf_calc observations.nc conf background_6565
	nice -n +10 mpirun -np $(NP_CALC) ./enkf_calc --forecast-stats-only stats.prm 2>&1 | tee calc.out

singleob-calc: enkf_calc ensemble_static
	nice -n +10 mpirun -np $(NP_CALC) ./enkf_calc --single-observation-ijk 30 50 0 SST 0.5 0.5 singleob.prm | tee calc.out

singleob-update: enkf_update ensemble_static
	nice -n +10 mpirun -np $(NP_UPDATE) ./enkf_update --output-increment singleob.prm | tee update.out
...
pointlogs-update: enkf_update X5.nc conf ensemble_6565
	nice -n +10 mpirun -np $(NP_UPDATE) ./enkf_update --no-fields-write pointlogs.prm 2>&1 | tee update-pointlogs.out

enkf-calc-global: enkf_calc observations.nc conf ensemble_6565
	nice -n +10 ./enkf_calc enkf-global.prm 2>&1 | tee calc.out

enkf-update-global: enkf_update X5.nc conf ensemble_6565
	nice -n +10 mpirun -np $(NP_UPDATE) ./enkf_update --calculate-spread --write-inflation enkf-global.prm 2>&1 | tee update.out
	rm -f ensemble_6565/mem*_*-???.nc

enkf_prep:
	cp /home/sindremf/PHD2/Work/enkf-c/enkf/bin/enkf_prep .	

enkf_calc:
	cp /home/sindremf/PHD2/Work/enkf-c/enkf/bin/enkf_calc .

enkf_update:
	cp /home/sindremf/PHD2/Work/enkf-c/enkf/bin/enkf_update .
